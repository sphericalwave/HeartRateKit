...
final class ScreenshotGenTests: XCTestCase {
    ...
    private var registry: [(name: String, size: CGSize, view: AnyView)] {
        [
            ...
            ("HRPill", CGSize(width: 180, height: 64),
             AnyView(HRPill(monitor: .preview()).padding())),
            ("LiveBPMLabel", CGSize(width: 180, height: 64),
             AnyView(LiveBPMLabel(monitor: .preview()).padding())),
        ]
    }
    ...
    /// `CompactHRChart` → `compact-hr-chart`, `LiveBPMLabel` → `live-bpm-label`.
